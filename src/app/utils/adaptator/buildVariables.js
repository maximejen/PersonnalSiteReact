    ...
    Object.keys(params.data).reduce((acc, key) => {
        ...
        if (isObject(params.data[key])) {
            ...
            if (Object.keys(fieldsToConnect).length === 0) {
                return acc;
            } else if (typeof fieldsToConnect.id === "string" || typeof fieldsToConnect.id === "number") { // if there is only one element to connect with.
            ...
            } else { // if multiple connect
                let tmp = [];
                for (let element in fieldsToConnect.id) {
                    tmp.push({id: fieldsToConnect.id[element]});
                }
                return {
                    ...acc,
                    data: {
                        ...acc.data,
                        [key]: {[PRISMA_CONNECT]: tmp}
                    }
                };
            }
        }
